...
import myket, { useMyket } from 'iab-myket-reactnative'

function PackagesScreen(props) {

    const myketInstance = useMyket("MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDLpqdcUrlnC+cljs9t79v6A/JlmeJLJmWRJMQWcFvTIuq5+D860KDW+rDF8onIEIXMWAl39YaEN87txDuHsP4sg063mYUYgzm7ET3fzn32lpi/TIiuNPCJwPr54+eY5Fj0SIIXS6RdXexILsrR0xZpYmpoZB1eYPnIP/5VOAd/VQIDAQAB");
    ...
    const onPayPressed = async (item) => {
        myketInstance.purchaseProduct(item.id.toString(), 'm' + Date.now().toString()).then((details) => {
            myketInstance.consumePurchase(details)
