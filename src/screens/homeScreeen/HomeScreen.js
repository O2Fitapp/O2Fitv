...
const HomeScreen = props => {
  ...
  const sendNotesToServer = async () => {
    const isSynced = await AsyncStorage.getItem("NoteSynced")
    if (isSynced == null) {
      const url = urls.foodBaseUrl + urls.note
      let notes = [];
      noteDB.allDocs({ include_docs: true }).then(res => {
        notes = res.rows.map(item => notes.push({ ...item.doc, userId: user.id }))
      })
  ...
  const onSuccessSyncNote = (res) => {
    AsyncStorage.setItem("NoteSynced", 'true')
  }

  const onFailureSyncNotes = () => {
