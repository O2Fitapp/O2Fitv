...
const DietPlanRow = ({ pack, meal, title, lang, onChangepackage, user, auth, selectedDate, fastingDiet, diet, icon, setDisableAdding, disableAddBtn, changePackageLoading }) => {
    ...
    const onSuccess = (params, index) => {
        ...
        if (pack.dietPackFoods.length - 1 == index) {
            ...
            const changeIsAte = { ...diet, [selectedDate]: { ...diet[selectedDate], [meal]: { ...diet[selectedDate][meal], isAte: true } },percent:diet.percent + 0.55 }
    ...
    const removeMealDB = (element, index) => {
        ...
        if (pack.dietPackFoods.length - 1 == index) {
            dispatch(setDietMeal({...removedmealFromServer,percent:diet.percent - 0.55}))
            setTimeout(() => {
                setLoading(false)
                setDisableAdding(false)
            }, 1000);
