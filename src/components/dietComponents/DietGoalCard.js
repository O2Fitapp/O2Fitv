...
const DietGoalCard = ({ diet, dietNew, fastingDiet, lang, navigation, profile, user, specification }) => {
    if (dietNew.isActive && dietNew.isOld == false) {
        return (
            <Pressable
                onPress={() => navigation.navigate("DietMainScreen", { nextRoute: "MyDietTab" })}
                style={styles.container}>
                <RowSpaceBetween style={styles.rowSpaceContainer}>
                    <Text style={[styles.text, { fontFamily: lang.titleFont }]}>هدف برنامه غذایی</Text>
                </RowSpaceBetween>
                {
                ...
                }
                {
                    ...
                    <RowSpaceBetween style={[styles.rowSpaceContainer, {}]}>
                        ...
                        <Text style={[styles.text, { fontFamily: lang.font }]}>{dietNew.cheetDays.length} {lang.day}</Text>
                    </RowSpaceBetween>
                }
                <RowSpaceBetween style={[styles.rowSpaceContainer, { borderTopWidth:  1, borderColor: defaultTheme.border, paddingTop: moderateScale(15) }]}>
        ...
        return (
            <Pressable
                ...
                <RowSpaceBetween style={[styles.rowSpaceContainer, {}]}>
                    <Text style={[styles.text, { fontFamily: lang.font }]}>{lang.usedCheetDays}</Text>
                    <Text style={[styles.text, { fontFamily: lang.font }]}>{diet.cheetDays.length} {lang.day}</Text>
                </RowSpaceBetween>
                <RowSpaceBetween style={[styles.rowSpaceContainer, { borderTopWidth: 1, borderColor: defaultTheme.border, paddingTop: moderateScale(15) }]}>
                    <Text style={[styles.text, { fontFamily: lang.font }]}>هدف برنامه</Text>
                    <Text style={[styles.text, { fontFamily: lang.font }]}>{specification[0].weightSize > profile.targetWeight ? "کاهش وزن" : profile.targetWeight < specification[0].targetWeight ? "افزایش وزن" : 'ثبات وزن'}</Text>
